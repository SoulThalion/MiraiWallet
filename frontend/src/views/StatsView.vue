  ...
  <div class="p-4 md:p-6 lg:p-8 max-w-screen-xl mx-auto">
      ...
      <button v-for="m in months" :key="m"
              :class="['flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-semibold transition-colors border',
                       selectedMonth === m
                         ...
                         : 'dark:bg-dark-card dark:border-white/[0.07] dark:text-dark-txt2 bg-light-card border-brand-blue/10 text-light-txt2']"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
      ...
      <div class="mw-card md:col-span-1">
        <p class="text-xs mb-4 dark:text-dark-txt2 text-light-txt2">Distribución — {{ selectedMonth }}</p>
      ...
      <div class="mw-card md:col-span-1 lg:col-span-2">
        <p class="text-xs mb-4 dark:text-dark-txt2 text-light-txt2">Gasto mensual — últimos 6 meses</p>
        <BarChart :bars="store.monthlyData" :maxVal="store.maxBar" />
      </div>

      <!-- Average cards -->
      <div class="mw-card">
        <p class="text-xs mb-1 dark:text-dark-txt2 text-light-txt2">Promedio mensual</p>
        <p class="font-display font-extrabold text-2xl md:text-3xl dark:text-dark-txt text-light-txt">
      ...
      <div class="mw-card">
        <p class="text-xs mb-1 dark:text-dark-txt2 text-light-txt2">Mejor mes</p>
        <p class="font-display font-extrabold text-2xl md:text-3xl dark:text-dark-txt text-light-txt">
          €{{ store.bestMonth.amount.toLocaleString('es-ES') }}
        </p>
        <p class="text-xs mt-1 dark:text-dark-txt2 text-light-txt2">{{ store.bestMonth.month }} 2024</p>
      </div>

      <div class="mw-card">
        <p class="text-xs mb-1 dark:text-dark-txt2 text-light-txt2">Total gastado</p>
        <p class="font-display font-extrabold text-2xl md:text-3xl dark:text-dark-txt text-light-txt">
          €{{ store.totalSpent.toLocaleString('es-ES') }}
        </p>
        <p class="text-xs mt-1 dark:text-dark-txt2 text-light-txt2">de €{{ store.totalBudget }} presupuestado</p>
      </div>

      <!-- Category breakdown — full width on all sizes -->
      <div class="mw-card md:col-span-2 lg:col-span-3">
        <p class="font-display font-bold text-sm mb-4 dark:text-dark-txt text-light-txt">Desglose por categoría</p>
