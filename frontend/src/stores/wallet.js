...
export const useWalletStore = defineStore('wallet', () => {
  ...
  const newExpense = ref({ description: '', category: 'Hogar', amount: '', date: '' })
  ...
  const saved = computed(() => totalIncome.value - totalExpenses.value)
  ...
  const donutSegments = computed(() => {
    ...
    return categories.value.map(cat => {
      ...
      const seg = { ...cat, pct: Math.round(pct * 100), dash, offset }
      offset += dash
      return seg
    })
  })

  const maxBar = computed(() => Math.max(...monthlyData.value.map(m => m.amount)))

  // ── Actions ────────────────────────────────────────────
  function toggleDark() { darkMode.value = !darkMode.value }
  function completeOnboarding() { onboarded.value = true }
  ...
  function addTransaction(tx) {
    ...
    balance.value -= Math.abs(parseFloat(tx.amount))
  }
  function updateBudget(categoryName, newBudget) {
